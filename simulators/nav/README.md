...
#### Running runsim
runsim is a bash script that can build required systems and run the 2d simulator. You can run the script with the command `bash runsim` directly in the simulators/nav directory of your shell (no need to vagrant up/ssh). Note: running `bash runsim -a` first will run `vagrant up`, `vagrant ssh`, and build and execute nav, lcm, and sim with jarvis. Later on, you can just exit vagrant and run `bash runsim` which will only run `vagrant ssh` and build nav, lcm, and sim. There are also options to specifically build + run a single package.
Usage: bash runsim [a|l|n|s]
        a) # (first time only) vagrant up + vagrant ssh + build and execute nav, lcm, and sim
        l) # vagrant ssh + build and execute lcm only
        n) # vagrant ssh + build and execute nav only
        s) # vagrant ssh + build and execute sim only
        default) # vagrant ssh + build and execute nav, lcm, and sim
