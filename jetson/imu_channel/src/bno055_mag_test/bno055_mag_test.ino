...
float mag_to_bearing(float mag_x, float mag_y) {
  float raw = atan2(mag_y, mag_x) * (180 / PI);
  if (raw < 0) {
    raw += 360.0;
  }
  return raw;
...
void loop(void)
{
  ...
  Serial.print(" mag bearing:");
  Serial.println(mag_to_bearing(magData.magnetic.x, magData.magnetic.y), 4);
