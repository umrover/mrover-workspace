...
#include "perception.hpp"
	...
	Camera::Impl::Impl() {
		...
		this->image_ = cv::Mat(this->image_size_.height, this->image_size_.width, CV_8UC4, this->image_zed_.getPtr<sl::uchar1>(sl::MEM::CPU));
		this->depth_zed_.alloc(this->image_size_.width, this->image_size_.height, sl::MAT_TYPE::F32_C1);
		this->depth_ = cv::Mat(this->image_size_.height, this->image_size_.width, CV_32FC1, this->depth_zed_.getPtr<sl::uchar1>(sl::MEM::CPU));
	}
	...
	Camera::Impl::~Impl() {
		...
		void Camera::Impl::dataCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr & p_pcl_point_cloud) {
			...
			for(auto &it : p_pcl_point_cloud->points) {
				...
				if(!isValidMeasure(X)) {
						it.x = it.y = it.z = it.rgb = 0; // Checking if it's a valid point
				}
				else {
						it.x = X;
	...
	Camera::Impl::Impl() {
		...
		std::cin >> path;
			...
			if(NULL == rgb_dir || NULL == depth_dir) return;
		#endif  
	...
	bool Camera::Impl::grab() {
		...
		cv::Mat Camera::Impl::depth() {
			...
			std::string full_path = depth_path + std::string("/") + rgb_name.substr(0, rgb_name.size()-4) + std::string(".exr");
			std::cout << full_path << std::endl;
			cv::Mat img = cv::imread(full_path.c_str(), cv::IMREAD_ANYCOLOR | cv::IMREAD_ANYDEPTH);
			if(!img.data) std::cerr << "Load image " << full_path << " error\n";
			return img;
		}
	...
	//Reads the point data cloud p_pcl_point_cloud
		...
		//Load in the file  
		if(pcl::io::loadPCDFile<pcl::PointXYZRGB> (full_path, *p_pcl_point_cloud) == -1) PCL_ERROR ("Couldn't read file test_pcd.pcd \n"); 
	}
	#endif
...
bool Camera::grab() {
	...
	void Camera::disk_record_init() {
		...
		if (-1 == system(mkdir_pcl.c_str()) || -1 == system( mkdir_rgb.c_str()) || -1 == system(mkdir_depth.c_str())) exit(1);
	}
	...
	void pcl_write(const cv::String &filename, pcl::PointCloud<pcl::PointXYZRGB>::Ptr &p_pcl_point_cloud){ 
		...
		try { 
			pcl::io::savePCDFileASCII (filename, *p_pcl_point_cloud); 
		}
