...
TagDetector::TagDetector() {  
    ...
    //throw error if dictionary file does not exist
    if (!fsr.isOpened()) {  
...
//gets coordinate of center of tag
Point2f TagDetector::getAverageTagCoordinateFromCorners(const vector<Point2f> &corners) {  
    //RETURN:
...
//detects AR tags in source Mat and outputs Tag objects for use in LCM
pair<Tag, Tag> TagDetector::findARTags(Mat &src, Mat &depthsrc, Mat &rgb) {  
    //RETURN:
    //pair of target objects- each object has an x and y for the center,
    //and the tag ID number return them such that the "leftmost" (x coordinate)
    //tag is at index 0
    ...
    //exactly two tags found
        ...
        //if tag 0 is left of tag 1, put t0 first
        if(t0.loc.x < t1.loc.x) {  
            discoveredTags.first = t0;
            discoveredTags.second = t1;
        } 
        //tag 1 is left of tag 0, put t1 first
        else {  
...
void TagDetector::updateDetectedTagInfo(rover_msgs::Target *arTags, pair<Tag, Tag> &tagPair, Mat &depthImg, Mat &src) {
    struct tagPairs {
        vector<int> id;
        vector<int> locx;
        vector<int> locy;
    ...
    for (uint i=0; i<2; i++) {
        //no tag found
        if(tags.id[i] == -1) {
            //send buffered tag until tag is found
            if(tags.buffer[i] <= 20) {
                ++tags.buffer[i];
            } 
            //if still no tag found, set all stats to -1
            else {
                arTags[i].distance = -1;
                arTags[i].bearing = -1;
                arTags[i].id = -1;
