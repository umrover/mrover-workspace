...
__global__ void find_clear_path(EuclideanClusterExtractor::Obstacle *obstacles, bool *heading_checks, float *min_dist_ptr, int obsArrSize)
{
  ...
  *min_dist_ptr = 3.4e+038;
  ...
  for (int j = 0; j < obsArrSize; ++j)
  { // Check all obstacles in obstacles array
    if (obstacles[j].minX < obstacles[j].maxX && obstacles[j].minZ < obstacles[j].maxZ)
    {
      ...
      if ((LBL_botLeft > 0 && RBL_botLeft < 0) || (LBL_botLeft < 0 && RBL_botLeft > 0) || LBL_botLeft == 0 || RBL_botLeft == 0)
      {
        heading_checks[i] = 0; // This is not a clear path
      }

      if ((LBL_botRight > 0 && RBL_botRight < 0) || (LBL_botRight < 0 && RBL_botRight > 0) || LBL_botRight == 0 || RBL_botRight == 0)
      {
        heading_checks[i] = 0; // This is not a clear path
      }

      if ((LBL_topLeft > 0 && RBL_topLeft < 0) || (LBL_topLeft < 0 && RBL_topLeft > 0) || LBL_topLeft == 0 || RBL_topLeft == 0)
      {
        heading_checks[i] = 0; // This is not a clear path
      }

      if ((LBL_topRight > 0 && RBL_topRight < 0) || (LBL_topRight < 0 && RBL_topRight > 0) || LBL_topRight == 0 || RBL_topRight == 0)
      {
        heading_checks[i] = 0; // This is not a clear path
      }
