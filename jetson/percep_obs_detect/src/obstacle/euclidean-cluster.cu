...
__device__ int
populateNeighborList(GPU_Cloud pc, float tolerance, Bins bins, int ptIdx, int partitions, float3 pt, int* list, int binNum, int count) {
    if ((binNum < 0) || (binNum >= partitions * partitions * partitions)) return -1;
    for (int j = 0; j < bins.data[binNum + 1] - bins.data[binNum]; ++j) {
        // Iterates through points in the bin
        float4 pcPointData = pc.data[j + bins.data[binNum]];
        float3 dvec = (pt - make_float3(pcPointData.x, pcPointData.y, pcPointData.z));
...
__global__ void
colorClusters(GPU_Cloud pc, int* labels, int* keys, int* values, int minCloudSize, int numClusters, float* minX, float* maxX, float* minY,
...
__global__ void
colorExtrema(GPU_Cloud pc, int* values, int minSize, int* labels, int numClustersOrig, int* validClustersCount, float* minX, float* maxX, float* minY,
             float* maxY, float* minZ, float* maxZ) {
...
//    std::cerr<<"Graph kernel built\n";
    ...
    auto pair = thrust::reduce_by_key(thrust::device, labelsSorted.begin(), labelsSorted.end(), count.begin(), keys.begin(),
                                      values.begin()); //remove duplicate labels and determine the number of points belonging to each label
    ...
    colorClusters<<<ceilDiv(pc.size, MAX_THREADS), MAX_THREADS>>>(pc, labels, gpuKeys, gpuVals, minSize, numClustersOrig, minX, maxX, minY, maxY,
                                                                  minZ, maxZ);
