...
{
    if ((binNum < 0) || (binNum >= partitions * partitions * partitions))
        return 0;
    int neighborCount = 0;
...
__global__ void colorExtrema(GPU_Cloud pc, int *values, int minSize, int *labels, int numClustersOrig, int *validClustersCount, float *minX, float *maxX, float *minY, float *maxY, float *minZ, float *maxZ)
{
    ...
    if (clusterIdx >= numClustersOrig)
        return;

    int place = 0;
    if (values[clusterIdx] > minSize)
        place = atomicAdd(validClustersCount, 1);
    else
        return;
