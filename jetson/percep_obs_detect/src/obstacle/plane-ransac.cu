...
__global__ void getOptimalModelPoints(GPU_Cloud pc, Plane &selection, int optimalCandidateIndex, int3 *candidatePlanePoints, int *maxInliersCount)
{
    ...
    float4 point;
    if (candidatePtNum == 0)
        point = pc.data[candidatePlanePoints[optimalCandidateIndex].x];
    if (candidatePtNum == 1)
        point = pc.data[candidatePlanePoints[optimalCandidateIndex].y];
    if (candidatePtNum == 2)
        point = pc.data[candidatePlanePoints[optimalCandidateIndex].z];
